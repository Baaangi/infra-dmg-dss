...
export default function AdminControlCenter() {
  ...
  useEffect(() => {
    // Fetch AI Config
    api.get('/settings/').then(res => setAiThreshold(res.data.confidence_threshold)).catch(() => { });
    // Fetch Personnel
  ...
  const handleAiUpdate = async () => {
    await api.put('/settings/', { confidence_threshold: aiThreshold });
    alert("AI Global Matrix Calibrated");
  };
              ...
              {users.map(user => (
                <tr key={user.id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right space-x-3">
                    ...
                    <button onClick={() => handleTerminate(user.id, user.username)} className="px-2 py-1.5 bg-rose-900/30 hover:bg-rose-600 text-rose-500 hover:text-white rounded transition-colors" title="Purge Account">
      ...
      {activeTab === "telemetry" && telemetry && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="glass rounded-xl p-8 border border-l-4 border-l-orange-500">
            <p className="text-zinc-500 text-xs font-medium uppercase tracking-widest mb-1">Globally Processed Audits</p>
      ...
      {activeTab === "ai" && (
        <div className="glass max-w-xl rounded-xl border border-white/5 p-8 relative overflow-hidden">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider mb-3">Model Confidence Threshold</label>
              <div className="flex items-center gap-6">
                ...
                />
                <span className="text-xl font-mono text-violet-400 bg-violet-500/10 px-3 py-1 rounded border border-violet-500/20">
